CREATE OR REPLACE PACKAGE pacote_relatorios AS
    ...
    PROCEDURE relatorio_pedidos_aceitos(p_id_fornecedor IN tb_fornecedor.id_fornecedor%TYPE);
    PROCEDURE calcular_total_itens_comprados(p_id_usuario IN tb_usuario.id_usuario%TYPE, p_total_itens OUT NUMBER);
END pacote_relatorios;


CREATE OR REPLACE PACKAGE BODY pacote_relatorios AS
    FUNCTION calcular_valor_total_compras(p_id_usuario IN NUMBER) RETURN NUMBER IS
        v_total NUMBER := 0;
    BEGIN
    ...
    FUNCTION calcular_preco_medio_produtos(p_id_fornecedor IN NUMBER) RETURN NUMBER IS
    ...
    PROCEDURE relatorio_pedidos_aceitos(p_id_fornecedor IN tb_fornecedor.id_fornecedor%TYPE) IS
    ...
    PROCEDURE calcular_total_itens_comprados(p_id_usuario IN tb_usuario.id_usuario%TYPE, p_total_itens OUT NUMBER) IS
    BEGIN
        SELECT COUNT(*)
        INTO p_total_itens
